...
string lpWindowName);
		...
		public ContinuousSpeech()
		{
			...
			ILoggerFactory loggerFactory = LoggerFactory.Create(builder =>
			{
				builder
									...
									.AddFilter("SpeechContinuousRecognition.Program", LogLevel.Debug);
			});
			_logger = loggerFactory.CreateLogger("ContinuousRecognition");
		...
		{
			string path = $"{Application.StartupPath}\\SpeechCommandLog{DateTime.Now.ToString("yyyy-MM-dd")}.txt";
			// Save text to A text file in the current Application directory with the current date.
			File.WriteAllText(path, $"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {value}");
		}
		...
		public int EmptyResultsToStopOn { get; } = 30;
		private void ContinuousSpeech_Load(object sender, EventArgs e)
		{
			...
			timerCommandTip.Interval = 30000;
			timerCommandTip.Enabled = true;
			...
			try
			{
				...
				if (result.Reason == ResultReason.RecognizedSpeech)
				{
					...
					if (this.TreatAsCommandFirst)
					{
						...
						{
							var commands = resultRaw.Split(new string[] { " split ", " Split " }, StringSplitOptions.TrimEntries);
							resultRaw = "";
							foreach (var item in commands)
							{
								var resultReturned = await _speechCommandsHelper.IdentifyAndRunCommandAsync(item, this, result, currentProcess);
								resultRaw = $"{resultRaw}{resultReturned}";
						...
						{
							resultRaw = await _speechCommandsHelper.IdentifyAndRunCommandAsync(lastCommand, this, result, currentProcess);
						}
						...
						{
							resultRaw = await _speechCommandsHelper.IdentifyAndRunCommandAsync(resultRaw, this, result, currentProcess);
						}
					}
		...
		private void timerCommandTip_Tick(object sender, EventArgs e)
		{
			DisplayRandomCommand();
		}
	}
